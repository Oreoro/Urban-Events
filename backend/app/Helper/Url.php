...
class Url
{
    ...
    public static function getCdnUrl(string $path): string
    {
        ...
        if ($disk === 'azure') {
            ...
            $configuredIncludesContainer = $configuredPath === $container
                || str_ends_with($configuredPath, '/'.$container);
            $containerBaseUrl = $configuredIncludesContainer
                ? $configuredUrl
                : $configuredUrl.'/'.$container;

            if ($container !== ''
                && $configuredUrl !== ''
                && str_starts_with($url, $configuredUrl.'/')
                && ! str_starts_with($url, $containerBaseUrl.'/')) {
